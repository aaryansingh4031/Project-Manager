...
const getWorkspaceStats = async (req, res) => {
  try {
    const { workspaceId } = req.params;
    if (!mongoose.Types.ObjectId.isValid(workspaceId)) {
      return res.status(200).json(emptyStatsResponse);
    }
...
const inviteUserToWorkspace = async (req, res) => {
  try {
    ...
    const inviteToken = jwt.sign(
      { user: existingUser._id, workspaceId: workspaceId, role: role || "member" },
    ...
    const emailContent = `<p>You have been invited to join ${workspace.name} workspace</p><p>Click here to join: <a href="${invitationLink}">${invitationLink}</a></p>`;
